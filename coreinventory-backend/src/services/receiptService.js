...
const validateReceipt = async (tenantId, receiptId, createdBy) => {
  ...
  // Increase stock for each item in a sequential loop
  // (each item is its own transaction, but we wrap the status update too)
  ...
  const updated = await receiptRepo.updateStatus(receiptId, 'validated', {
